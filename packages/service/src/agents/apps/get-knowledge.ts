...
import type { KnowledgeDetailItem } from '../tools/get-knowledge';
import { AgentBase } from '../modules';

export class AgentGetKnowledge extends AgentBase {
  /** @override */
  static agentName = 'get-knowledge';
...
<todo>
你会获得以下输入信息：
- 若干条知识库搜索结果，格式参考下面 \`<knowledge-example>\` 的 XML 格式
- 用户对话信息

你的任务：
- 首先从用户对话信息中提取出[用户当前关注主题]
- 判断每条搜索结果与[用户当前关注主题]是否相关，排除所有不相关的搜索结果
- 改写每条搜索结果，将详细内容部分**与[用户当前关注主题]相关**的内容提取出来，作为新的详细内容
...
<output-rules>
生成 XML 输出的要求：
- 你必须严格按照下面 \`<knowledge-example>\` 的 XML 格式返回你的分析结果
- 输出的 XML **禁止**包含任何额外的解释或文本
- **禁止**将 XML 包裹在 markdown 代码块中（不要使用\`\`\`或\`\`\`xml）
- 每个开头 <knowledge ...> 标签距离详细内容的开始部分至少要有两个换行符
</output-rules>

<knowledge-example>

${this._knowledgeXmlPrompt([
  {
...
- 标签校验：数清<knowledge>的数量与</knowledge>的数量是否完全一致，无多漏；
- 引号校验：检查title、url、project-name三个属性的双引号，是否每个都“开头有"、结尾有"”，无半开状态；
- 符号校验：检查所有标签是否为<knowledge ...>（开头）、</knowledge>（结尾），无<knowledge>后漏“>”、闭合标签漏“/”等情况。
</xml-generate-checklist>
`;
  }
...
${this._knowledgeXmlPrompt(context.inputs.rawKnowledgeResult)}
...
${messagePrompt}
  ...
  private _knowledgeXmlPrompt(knowledgeResult: KnowledgeDetailItem[]) {
    return knowledgeResult.map(
      res => `<knowledge title="${res.title}" url="${res.url}" project-name="${res.projectName}">\n\n${res.content}\n\n</knowledge>`,
    ).join('\n');
  }
}
